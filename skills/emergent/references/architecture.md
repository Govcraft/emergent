...
Every SDK subscribes to `system.shutdown` on its own and swallows it, so it is three events in the event store and none in your handler. It closes the handler/sink stream whose `kind` matches. On SDK release 0.13.1 and earlier only the Go SDK did: the engine delivers the whole message envelope as the notification payload, which puts `kind` one level deeper than the Rust, Python and TypeScript SDKs looked for it, so a sink there logs `shutdown_kind=unknown` on all three broadcasts and stops only on the SIGTERM that follows (Govcraft/emergent#43). After 0.13.1 all four SDKs read the inner envelope, and a Rust, a Python and a TypeScript sink each closed its stream within 60 ms of the `{"kind":"sink"}` broadcast, well inside the 500 ms before SIGTERM. Make SIGTERM a clean exit anyway: it is still the fallback, and it is where a primitive on 0.13.1 or earlier ends.
