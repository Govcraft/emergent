...
policy authorized, reported through `PolicyObserver::on_subscribed`; the
connection is attributed to a primitive by walking the peer's pid up its process
ancestry to a live child of the engine, so a primitive connecting from a
grandchild process (`uv run`, `sh -c`) is named and releases its tier in
milliseconds instead of costing the deadline. On 0.10.10 and earlier the
...
- `ipc_identity.rs`: `ConnectionIdentity`, the process-ancestry resolver admission asks who a peer is, and the registry that revokes a dead primitive's connections
...
- `[engine]` also: `enforce_declarations`, `authenticate_connections`, `startup_ready_timeout_ms`, `shutdown_drain_ms`, `shutdown_grace_ms`
...
Declaration enforcement: `[engine].enforce_declarations` is `"off"`, `"warn"` or `"strict"`, default `"off"`. After engine 0.10.10 it decides whether a primitive's `publishes` list binds it. `"warn"` logs a publish outside the declarations at WARN with the primitive, operation and message type; `"strict"` also refuses it, so the message is neither stored nor forwarded, the client gets an `ACCESS_DENIED` error carrying the engine's sentence, and the engine emits `system.error.<name>` with the reason. Matching is the same exact-or-trailing-wildcard rule as subscriptions. Protocol topics are always allowed on the operation they belong to: publishing `system.request.subscriptions` and `system.request.topology`, subscribing to `system.response.subscriptions`, `system.response.topology` and `system.shutdown`, all of which the SDKs do for you before your code runs. The engine's own `system.*` events arrive as `IpcSystemEvent` and never pass the check. On 0.10.10 and earlier the lists were advisory and nothing was checked. Enforcement runs in an `IpcSecurityPolicy` (acton-reactive 9.4.0), installed only when the mode is `warn` or `strict`, in `emergent-engine/src/ipc_policy.rs`; the decisions are pure functions in `emergent-engine/src/declarations.rs` and the lookup table is built once from config. The name a check is made under is the engine's own answer, not the client's: `emergent-engine/src/ipc_identity.rs` resolves a connection at admission by walking the peer's pid up its process ancestry (bounded at 16 generations, `/proc` on Linux) to a live child of the engine, so a primitive behind `uv run` or `sh -c` is named by the child the engine spawned. A publish whose `source` names a different primitive than its connection is itself a violation; a connection the engine cannot tie to a primitive gets the protocol topics and nothing else, in either mode, and cannot borrow a configured name; subscriptions are checked because the name comes from the connection rather than the frame. A pid is a primitive only while the engine holds that child as live, which is what makes pid reuse safe. When a child exits, every connection admitted under its name is revoked through `IpcListenerHandle::revoke_connection`, which matters for a forking wrapper whose descendant outlives it. `[engine].authenticate_connections` (`"off"`, `"warn"`, `"strict"`, default `"off"`) governs only what happens to a peer the engine did not spawn: `strict` admits a same-uid peer, because the CLI, the topology viewer and a hand-run primitive are indistinguishable from an impostor at admission, and refuses a different uid, which the `0660` socket otherwise lets in. Where `/proc` is unavailable an unnamed connection falls back to the `source` on the message and the engine says so at startup (issue #24).
