...
- **Wire Format**: MessagePack, always. `[engine].wire_format` still parses
  ...
  0.10.10 and earlier it was inert and unannounced; from 0.14.0 setting it
...
stream is dropped. From 0.14.0 the engine will not start a topology
whose enabled primitives, plus 4 reserved connections, exceed the effective
limit.

**Declarations bind only when told to.** From 0.14.0,
...
always on the wire. From 0.14.0 the Rust SDK claims that answer and
...
The type is concrete, one per primitive, so naming each one is always correct: `subscribes = ["system.error.fetch-issues", "system.error.score-severity"]`. From 0.14.0 a sink that wants every lifecycle event can subscribe to `system.error.*` instead of listing every primitive. On 0.10.10 and earlier that subscription received nothing, so those topologies list each primitive.
...
| `system.error.<name>` | `{name, kind, pid, publishes, subscribes, error}` | Any other exit (`error` is `"Exited with status: N"`), or the spawn itself failed (`error` is the OS error text and there is no `pid`). From 0.14.0 also `"Restarts exhausted: N attempts within W ms"` when a restart policy gives up |
| `system.restarted.<name>` | `{name, kind, pid, publishes, subscribes, restart_attempt}` | From 0.14.0 only. The restart policy respawned the primitive. `restart_attempt` is 1-based inside the current window. It follows the `system.error.<name>` or `system.stopped.<name>` that reported the exit |
...
| `system.request.topology` | Topology query via pub/sub. Answered by the engine from 0.14.0; 0.10.10 and earlier never answer it |
...
Within each tier, primitives start in config order. From 0.14.0 the engine then waits for that tier before starting the next: it holds until every primitive in it that declares `subscribes` has reached the engine over IPC, so a slow-starting consumer no longer misses a source's first event. A primitive that declares no `subscribes`, every source among them, is never waited on. `[engine].startup_ready_timeout_ms` (default 5000) bounds the wait; at the deadline the engine logs a warning naming the primitives it never heard from and starts the next tier anyway, and a primitive that exits or fails during the wait releases its tier at once.

The wait is a real signal, not an inference: the engine's IPC security policy is told about every subscribe before acton registers it, and startup waits on that. It fires whether a primitive asks the engine for its configured `subscribes` or passes its topics in code. What is still approximate is naming the primitive behind a subscribe. Until the engine resolves a peer to the process it spawned, the policy reports the peer's kernel pid and startup matches it against its own children, so a primitive running behind a wrapper that forks (`uv run`, for instance) connects from a grandchild the engine cannot name and costs its tier the deadline.
...
**Supervision is opt-in, and absent on 0.10.10 and earlier.** By default the engine does not restart a primitive that exits or fails to spawn. It emits `system.stopped.<name>` or `system.error.<name>` and the primitive stays down until the engine restarts. From 0.14.0 a primitive can set `restart = "on-failure"` or `"always"` (see the configuration reference): the engine respawns it with a doubling backoff, emits `system.restarted.<name>`, and gives up with a `system.error.<name>` of `"Restarts exhausted: ..."` once `restart_max_retries` restarts land inside `restart_window_ms`. It never restarts anything during shutdown.
...
**From 0.14.0** two things stop them, and the second one is what covers platforms the first does not.
