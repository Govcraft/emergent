...
| `authenticate_connections` | String | `"off"` | From 0.14.0. What becomes of an IPC connection from a process the engine did not spawn: `"off"` admits it, `"warn"` admits it with a WARN, `"strict"` admits it only if it runs as the same user |
