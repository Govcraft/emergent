...
| `Discover(ctx)` | List the IPC type names and IPC-exposed actors the engine reports (`SystemEvent`, `message_broker`). These are not Emergent topics or primitives: use `GetTopology` on a sink, or `GET /api/topology`, for those |
